...
AutoPeerBackend.prototype._onNewPeerConnection = function (newClientId) {
  ...
  _this.sendTo(newClientId, 'autoPeer:connected', newClientId);
  // Send to all but the new client that there is a new client
  _this.getClientIds(key).forEach(function (clientId) {
    if (clientId !== newClientId) {
      _this.sendTo(clientId, 'autoPeer:peerCollectionUpdate', _this.getClientIds(), key);
    }
  });
...
/**
 * Returns all clients of the given key - if no key is specified the default key is used
 *
 * @param key optional
 * @returns {*|Array}
 */
AutoPeerBackend.prototype.getClientIds = function(key) {
  return Object.keys(this.clients[key || this.defaultKey] || {});
};
