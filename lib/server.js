...
AutoPeerBackend.prototype.init = function () {
  ...
  // Attach custom transmission handler
  var origHandleTransmission = _this.peerServer._handleTransmission;
  _this.peerServer._handleTransmission = function () {
    if (!_this._handleTransmission.apply(_this, arguments)) {
      return origHandleTransmission.apply(this, arguments);
    }
  };
...
AutoPeerBackend.prototype._handleTransmission = function (key, socketMessage) {
  // Check if this transmission should be handled by auto-peer
  if(typeof socketMessage !== 'object' ||
    typeof socketMessage.payload !== 'object' ||
    socketMessage.payload.type !== 'auto-peer' ||
    !socketMessage.payload.message.event) {
    // Invalid message - fallback to peerJS
    return false;
  }
  console.log('emit', socketMessage.payload.message);
  this._emitMessage(socketMessage.payload.message);
  return true;
};
 ...
 * @param message
 */
AutoPeerBackend.prototype._emitMessage = function (message) {
